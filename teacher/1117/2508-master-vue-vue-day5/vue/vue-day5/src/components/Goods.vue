...
export default {
  ...
  methods: {
        ...
        item.foods.forEach((food, i) => {
          ...
          if (food.name === name) {
            if (food.count) {
              ...
              // 数量为 0，把食物从购物车列表中删除
              if (food.count === 0) {
                const index = this.cartList.findIndex(v => v.name === name)
                if (index > -1) {
              ...
              food.count = 1
              // 购物车列表中没有找到数据再 push
              if (!this.cartList.find(v => v.name === name)) {
    ...
    changeFloor (index) {
      ...
      this.$refs.tabContent.scrollTop = this.$refs.tabContent.children[index].offsetTop
      // 点击左侧按钮是添加标记，1s 之内不要触发滚动事件
      this.isClick = true
    ...
    scrollRight (e) {
      ...
      const index = this.offsetTops.findIndex((v, i) => {
        ...
        return e.target.scrollTop >= v && e.target.scrollTop < this.offsetTops[i + 1]
      })
    ...
    <div class="tab-wrap">
      <div class="tab-nav">
        <div
          ...
          :key="item.name"
          :class="['tab-nav-item', { active: activeIndex === index }]"
      ...
      <div class="tab-content" ref="tabContent" @scroll="scrollRight">
        ...
        >
          <h3>{{ item.name }}</h3>
          <ul>
            <li v-for="food in item.foods" :key="food.name" class="food">
              <img :src="food.icon" width="57" height="42" @click="foodDetail = food" alt="">
              <div class="food-info">
                <h4>{{ food.name }}</h4>
                <p>月售 {{ food.sellCount }} 份，好评率${{ food.rating }}%</p>
                <div class="price">¥{{ food.price }}</div>
                <div class="btns">
                  <template v-if="food.count > 0">
                    <button @click="changeCount(food.name, -1)">-</button>
                    {{ food.count }}
                  </template>
                  <button @click="changeCount(food.name, 1)">+</button>
                </div>
