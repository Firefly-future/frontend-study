...
app.get('/api/article/list', checkAuth, (req, res) => {
  // 读取数据库
  const articleList = readFile('articles.json').filter(v => !v.isDel)
  const userList = readFile('userlist.json')
  res.send({
    code: 200,
    message: '成功',
...
// 删除文章
app.post('/api/article/remove', checkAuth, (req, res) => {
  const { id } = req.body
  // 读取数据库
  const articleList = readFile('articles.json')
  // 根据 id 查找文章是否存在
...
// 编辑文章
app.put('/api/article/update', checkAuth, (req, res) => {
  ...
  articleList[index].updateTime = new Date()
  // 写入文件
  writeFile('articles.json', JSON.stringify(articleList))
  res.send({
    code: 200,
