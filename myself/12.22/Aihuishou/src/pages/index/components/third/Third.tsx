...
const Third = ({
  ...
  if (!inquery?.[2]?.items) return null
  const unRecyle = inquery?.[2].items.filter(
    (item) =>
  ...
  console.log(activeId4)
  return (
    <div className={style.third}>
      <h4>
        {" "}
        <span>4.{inquery?.[2].name}</span>{" "}
        <b onClick={() => setCollapsed(!collapsed)}>
          {" "}
          {activeId4 === ""
            ? ""
            : inquery?.[2].items.find((item) => item.id === activeId4)?.name}
          {activeId4 === "" ? "" : <FormOutlined />}
        </b>
      </h4>
      {!collapsed && (
        <ul>
          {inquery?.[2].items.map((item, ind) => {
            return (
              <li
                key={item.id}
                className={classNames(
                  style.index_item,
                  activeId4 === item.id ? style.active : "",
