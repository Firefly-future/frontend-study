...
const Fourth = ({
  ...
  if (!inquery?.[3]?.items) return null
  const unRecyle = inquery?.[3].items.filter(
    (item) =>
  ...
  const handleClick = (item: any) => {
    ...
    setActiveId5(item.id)
    setCollapsed(true)
    // setCurrent(5)
  }
  console.log(activeId5)
  return (
    <div className={style.fourth}>
      <h4>
        {" "}
        <span>5.{inquery?.[3].name}</span>{" "}
        <b onClick={() => setCollapsed(!collapsed)}>
          {" "}
          {activeId5 === ""
            ? ""
            : inquery?.[3].items.find((item) => item.id === activeId5)?.name}
          {activeId5 === "" ? "" : <FormOutlined />}
        </b>
      </h4>
      {!collapsed && (
        <ul>
          {inquery?.[3].items.map((item, ind) => {
            return (
              <li
                key={item.id}
                className={classNames(
                  style.index_item,
                  activeId5 === item.id ? style.active : "",
