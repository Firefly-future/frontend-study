...
import './player.scss'
import {$,$all,getQuery,Int,formatDuration,formatLyricTime} from '@util/util.js'
import {
    getBoundApi,
    getRedHeartCount,
    getCommentCount,
...
async function getBound() {
    try {
        const res = await getBoundApi(ids)
        // console.log(res)
...
async function getHotheart() {
    try {
        const res = await getRedHeartCount(id)
        console.log(res)
...
async function getSongUrl(){
    try{
        const res=await getSongUrlApi(id)
        // console.log(res)
